...
NSString* const Xbox360ControllerActivityNotification = @"Xbox360ControllerActivityNotification";
...
- (Xbox360ControllerManager*) init {
  ...
  if (self) {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(HIDActivity:) name:Xbox360HIDActivityNotification object:NULL];
  }
  return self;
}

- (void) HIDActivity:(NSNotification*)notification {
  // Fetching the data
  ...
  [[NSNotificationCenter defaultCenter] postNotificationName:Xbox360ControllerActivityNotification object:dictionary];
}
